...
class PongEnv:
    def __init__(self, render_mode=None, target_fps = 60):
        ...
        self.paddle_width = 0.5  # 패들 너비 (화면 대비 비율)
    ...
    def step(self, action):
        ...
        if action == 0:
            self.paddle_x -= 0.05 #왼쪽
        elif action == 2:
            self.paddle_x += 0.05 #오른쪽
    ...
    def _get_state(self):
        ...
        """
        return np.array([self.ball_x, self.ball_y, self.paddle_x, self.ball_dx, self.ball_dy], dtype = np.float32)
            # 여기에 5개의 값을 채워넣으세요
